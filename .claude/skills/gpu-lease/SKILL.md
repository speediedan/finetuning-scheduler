...
this repo, sessions in a sibling project (this project shares its host and self-hosted CI agent with
`interpretune`), and the self-hosted Azure Pipelines agent, which can dispatch a GPU job the moment one is
approved.
...
`tests/special_tests.sh` and `scripts/gen_fts_coverage.sh` then **self re-exec** under the lease, so a whole suite
is covered by one acquisition. For anything else that touches the GPU, wrap it explicitly:
...
| ------------------------------------------------------- | -------------------------------------------------- | --------------------------------------------------------------- |
| `HELD` with `[container]` and `project=azure-<buildId>` | a CI job holds it                                  | **Cancel the pipeline run** — see the warning below             |
| `HELD` by the sibling project                           | legitimately in use                                | Wait or coordinate; do not reset                                |
