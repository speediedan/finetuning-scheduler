...
class ModelParallelStrategyAdapter(StrategyAdapter):
    ...
    def on_before_init_fts(self) -> None:
        ...
        4. Configure FTS wrapping of the provided :external+pl:class:`~lightning.pytorch.core.module.LightningModule`
           ...
           ``auto_wrap_policy``.
        """
        # # hack to avoid subclassing ModelParallelStrategy strategy for this adapter
        ...
        # self._maybe_squeeze_device_mesh()  # TODO: not currently useful, probably remove before releasing
        pass
        #setattr(self.pls_handle, "_setup_device_mesh", ModelParallelStrategyAdapter._setup_device_mesh)
    ...
    # TODO: not currently useful, probably remove before releasing
    # def _maybe_squeeze_device_mesh(self) -> None:
    #     from torch.distributed.device_mesh import init_device_mesh
    #     assert self.pls_handle.device_mesh
    #     if self.pls_handle._tensor_parallel_size == 1 and self.pls_handle._data_parallel_size > 1:
    #         self.pls_handle._device_mesh = init_device_mesh(
    #             device_type=self.pl_module.device.type, mesh_shape=(self.pls_handle._data_parallel_size,),
    #             mesh_dim_names=("data_parallel",))
    #     elif self.pls_handle._tensor_parallel_size > 1 and self.pls_handle._data_parallel_size == 1:
    #         self.pls_handle._device_mesh = init_device_mesh(
    #             device_type=self.pl_module.device.type, mesh_shape=(self.pls_handle._tensor_parallel_size,),
    #             mesh_dim_names=("tensor_parallel",))
    #     elif self.pls_handle._tensor_parallel_size == 1 and self.pls_handle._data_parallel_size == 1:
    #         raise MisconfigurationException(
    #             "When using model parallel, either `tensor_parallel_size` or `data_parallel_size` should be > 1.")
    #     # TODO: determine if we really need to block until all ranks have re-initialized the device mesh, probably safe
    #     self.pls_handle.barrier()
